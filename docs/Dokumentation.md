...
Damit alles offline läuft und sicher ist, habe ich den Server selbst aufgesetzt.

### 3.1 Das System
Die Software ist in drei einfache Teile aufgeteilt:
1. **Coolify Dashboard:** Das ist eine Webseite auf dem Server. Hier lade ich den Code hoch und steuere die Programme.
2. **Docker:** Docker packt die Programme in sichere, getrennte Boxen (Container), damit sie sich nicht stören.
3. **SchoolAI Programme:** Das sind die Webseite, die Datenbank und das KI-Modell.

### 3.2 Sicherheit (Damit niemand hackt)
...
* **Webseite:** Programmiert mit **Next.js** und **React**. Für das Design habe ich **Tailwind CSS** benutzt und ein cooles Design gebaut (dunkler Hintergrund mit lila und blauen Leuchteffekten).
...
* **KI-Modell:** **Ollama** läuft lokal auf meiner Grafikkarte. Das Modell basiert auf Llama 3 und ist so eingestellt, dass es sehr ruhig und logisch antwortet.
...
1. **Chats mitlesen:** Alle geschriebenen Unterhaltungen ansehen, um zu kontrollieren, ob Schüler die KI richtig nutzen.
2. **Regeln einstellen:** Per Knopfdruck festlegen, ob der Bot Aufsätze blockieren, fertige Hausaufgaben verbieten oder stur nur mit Gegenfragen antworten soll.
...
## 8. Wo man meinen Code findet (für die Lehrer)

Da Frau Schwab und Herr Kamm meinen Code anschauen wollen, habe ich hier aufgeschrieben, wo meine Programmierarbeiten liegen. Es ist alles in einem großen Ordner (ein Monorepo) aufgeteilt:

* **Der Code für die Webseite**: Liegt im Ordner `apps/web/src/app/api/chat/route.ts`. Hier drin steht die Logik, wie der Bot dem Schüler antwortet und die Lehrer-Regeln beachtet.
* **Die Datenbank-Struktur**: Liegt im Ordner `packages/database/prisma/schema.prisma`. Hier habe ich die Tabellen für Benutzer, Chats und Nachrichten definiert.
* **Das Lehrerportal**: Liegt im Ordner `apps/web/src/app/teacher/page.tsx`. Das ist der Code für die Lehrerseite mit dem PIN-Schutz `4545`.
* **Die KI-Einstellungen**: Liegt in der Datei `Modelfile.apertus`. Hier steht der Text (System-Prompt), der dem Bot verbietet, Lösungen vorzusagen.

### Wie man das Programm startet:
1. `docker compose up -d` (startet die Hintergrund-Programme)
2. `npx prisma db push` (richtet die Datenbank ein)
